...
BEGIN
    ...
    LOOP
        ...
        -- matching company, date, material (case-insensitive) and submaterial (case-insensitive)
        -- Uses IS NOT DISTINCT FROM to correctly handle NULLs
        SELECT id INTO existing_id
        FROM analysis_records
        WHERE company_id = rec_company_id
          AND date IS NOT DISTINCT FROM rec_date
          AND LOWER(material) IS NOT DISTINCT FROM LOWER(rec_material)
          AND LOWER(sub_material) IS NOT DISTINCT FROM LOWER(rec_submaterial);
          
        IF existing_id IS NOT NULL THEN
            -- Update existing record
            -- COALESCE ensures we only update if new value is provided (not null)
            -- If new value is null (key missing in JSON or explicit null), keep existing value
            -- This effectively merges the new data with existing data column by column
